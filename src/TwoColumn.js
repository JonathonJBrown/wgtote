...
const Store = {
  async save(k,d){try{localStorage.setItem(`wg2:${k}`,JSON.stringify(d))}catch{}},
  async load(k,fb){try{const s=localStorage.getItem(`wg2:${k}`);return s?JSON.parse(s):fb}catch{return fb}},
};
...
export default function TwoColumnApp() {
  ...
  const restoreBackup = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (data.teams) setTeams(data.teams); if (data.scores) setScores(data.scores);
        if (data.notes) setNotes(data.notes);
        if (data.judgeCap) setJudgeCap(data.judgeCap); if (data.customClasses) setCustomClasses(data.customClasses);
        if (data.eventDate) setEventDate(data.eventDate); if (data.eventVenue) setEventVenue(data.eventVenue);
        if (data.judgeName) setJudgeName(data.judgeName);
        dirty(); setImportMsg("Backup restored successfully.");
  ...
  const exportXlsx = async () => {
    ...
    for (const tote of totes) {
      ...
      ranked.forEach((t, i) => {
        const row = [ordinal(i + 1), t.name, t.s1, ordinal(s1sorted.indexOf(t.id) + 1), t.s2, ordinal(s2sorted.indexOf(t.id) + 1), fmtTotal(t.tot), ordinal(totSorted.indexOf(t.id) + 1)];
    ...
    <div style={{maxWidth:1200,margin:"0 auto",padding:"4px 8px"}}>
      ...
      {tab==="settings"&&<div style={{animation:"fadeIn .2s"}}>
        ...
        <Card>
          ...
          <p style={{fontSize:14,color:"var(--t2)",marginBottom:6}}>Download a backup file to protect your data. Restore from a backup if needed.</p>
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,backdropFilter:"blur(4px)"}} onClick={()=>setShowHelp(false)}>
        <div onClick={e=>e.stopPropagation()} style={{background:"var(--s1)",border:"1px solid var(--bd)",borderRadius:10,padding:20,width:"92%",maxWidth:520,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 8px 32px rgba(0,0,0,.5)"}}>
          ...
          <div style={{fontSize:15,color:"var(--t2)",lineHeight:1.6}}>
            ...
            <p><b>Ranking table</b> (bottom left) — Teams ranked by total with ordinals under each score.</p>
            <p><b>Notes</b> (bottom right) — A text field for each team for commentary during performances.</p>
