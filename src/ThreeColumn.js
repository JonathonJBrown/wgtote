    ...
    <div style={{maxWidth:1200,margin:"0 auto",padding:"4px 8px"}}>
      ...
      {tab==="settings"&&<div style={{animation:"fadeIn .2s"}}>
        ...
        <Card>
          ...
          <p style={{fontSize:13,color:"var(--t2)",marginBottom:6}}>Download a backup file to protect your data. Restore from a backup if you need to recover.</p>
        ...
        {curTeams.length===0?<p style={{textAlign:"center",padding:24,color:"var(--t3)",fontSize:13}}>No teams.</p>:<div>
          <div style={{display:"flex",flexDirection:"row",gap:6}}>
            ...
            <div style={{minWidth:0,flex:"0 0 auto",width:Math.max(160, Math.min(260, curTeams.length > 6 ? 180 : 220)),display:"flex",flexDirection:"column"}}>
              <div style={{textAlign:"center",marginBottom:2}}>
                <div style={{fontFamily:"'DM Sans',sans-serif",fontSize:17,lineHeight:1.1}}>Ranking</div>
                <div style={{fontSize:10,color:"var(--t3)"}}>by total</div>
              </div>
              <div style={{background:"var(--s1)",borderRadius:7,border:"1px solid var(--bd)",padding:6,flex:1,overflowY:"auto",maxHeight:trackH}}>
                {(()=>{
                  ...
                  const s2r = [...rows].sort((a,b)=>b.s2-a.s2).map(t=>t.id);
                  return (<div>
                    {factors && <div style={{fontSize:9,color:"var(--ac)",marginBottom:3,textAlign:"center"}}>{factors.label} factoring</div>}
                    {rows.map((t,i)=>(
                      <div key={t.id} style={{display:"flex",alignItems:"flex-start",gap:4,padding:"4px 4px",borderBottom:"1px solid var(--bd)",fontSize:12}}>
                        <div style={{textAlign:"center",minWidth:20,flexShrink:0,paddingTop:2}}>
                          <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:16,height:16,borderRadius:"50%",fontSize:10,fontWeight:700,background:i===0?"#F59E0B":i===1?"#94A3B8":i===2?"#CD7F32":"var(--s3)",color:i<3?"#000":"var(--t2)"}}>{i+1}</span>
                          <div style={{fontSize:9,color:"var(--t3)"}}>{ordinal(i+1)}</div>
                        </div>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontWeight:500,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",fontSize:12,marginBottom:2}}>{t.name}</div>
                          <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"var(--t3)"}}>
                            <div style={{textAlign:"center"}}>
                              ...
                              <div style={{fontSize:9,color:"var(--t3)"}}>{ordinal(s1r.indexOf(t.id)+1)}</div>
                            </div>
                            <div style={{textAlign:"center"}}>
                              <div style={{color:boxFor(t.s2).color,fontWeight:600}}>{t.s2}</div>
                              <div style={{fontSize:9,color:"var(--t3)"}}>{ordinal(s2r.indexOf(t.id)+1)}</div>
                            </div>
                            <div style={{textAlign:"center"}}>
                              <div style={{fontWeight:600,color:"var(--t1)"}}>{fmtTotal(t.tot)}</div>
                              <div style={{fontSize:9,color:"var(--t3)"}}>{ordinal([...rows].sort((a,b)=>b.tot-a.tot).findIndex(x=>x.id===t.id)+1)}</div>
                            </div>
          ...
          <div style={{background:"var(--s1)",border:"1px solid var(--bd)",borderRadius:7,padding:10,marginTop:8}}>
            <h3 style={{fontFamily:"'DM Sans',sans-serif",fontSize:16,marginBottom:6}}>Notes</h3>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead>
                <tr>
                  <th style={{textAlign:"left",padding:"4px 6px",borderBottom:"2px solid var(--bd)",color:"var(--t2)",fontWeight:600,fontSize:12,textTransform:"uppercase",width:140}}>Team</th>
                  <th style={{textAlign:"left",padding:"4px 6px",borderBottom:"2px solid var(--bd)",color:"var(--t2)",fontWeight:600,fontSize:12,textTransform:"uppercase"}}>Comments</th>
                </tr>
              </thead>
              <tbody>
                {curTeams.map(t=>(
                  <tr key={t.id}>
                    <td style={{padding:"3px 6px",borderBottom:"1px solid var(--bd)",fontSize:13,fontWeight:500,verticalAlign:"top",whiteSpace:"nowrap"}}>{t.name}</td>
                    <td style={{padding:"2px 4px",borderBottom:"1px solid var(--bd)"}}>
                      <textarea
                        ...
                        placeholder="..."
                        rows={1}
                        style={{width:"100%",padding:"3px 6px",borderRadius:3,border:"1px solid var(--bd)",background:"var(--s2)",color:"var(--t1)",fontSize:13,fontFamily:"'DM Sans',sans-serif",outline:"none",resize:"vertical",lineHeight:1.3,minHeight:24}}
                      />
                    </td>
                  </tr>
                ))}
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,backdropFilter:"blur(4px)"}} onClick={()=>setShowHelp(false)}>
        <div onClick={e=>e.stopPropagation()} style={{background:"var(--s1)",border:"1px solid var(--bd)",borderRadius:10,padding:20,width:"92%",maxWidth:520,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 8px 32px rgba(0,0,0,.5)"}}>
          ...
          <div style={{fontSize:14,color:"var(--t2)",lineHeight:1.6}}>
            ...
            <h3 style={{fontFamily:"'DM Sans',sans-serif",fontSize:18,color:"var(--t1)",marginBottom:4,marginTop:14}}>Notes</h3>
