...
const Store = {
  async save(k,d){try{localStorage.setItem(`wg4:${k}`,JSON.stringify(d))}catch{}},
  async load(k,fb){try{const s=localStorage.getItem(`wg4:${k}`);return s?JSON.parse(s):fb}catch{return fb}},
};
...
/* ═══ APP ═══ */
export default function FourColumnApp() {
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,backdropFilter:"blur(4px)"}} onClick={()=>setShowHelp(false)}>
        <div onClick={e=>e.stopPropagation()} style={{background:"var(--s1)",border:"1px solid var(--bd)",borderRadius:10,padding:20,width:"92%",maxWidth:520,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 8px 32px rgba(0,0,0,.5)"}}>
          ...
          <div style={{fontSize:15,color:"var(--t2)",lineHeight:1.6}}>
            ...
            <p>Each tote has four side-by-side columns:</p>
            <p style={{marginTop:4}}><b>Column 1</b> — Sub-caption 1 scale (Vocabulary, Composition, or Repertoire depending on your caption).</p>
            <p><b>Column 2</b> — Sub-caption 2 scale (Excellence or Performance).</p>
            <p><b>Column 3</b> — Live ranking panel showing each team sorted by total, with ordinals under each score.</p>
            <p><b>Column 4</b> — Notes with a text field for each team for commentary during performances. Notes auto-save and are included in exports.</p>
